...
search = st.text_input("Type any indicator name to view its meaning:")

if search:
    # Case-insensitive matching
    matches = {k: v for k, v in definitions.items() if search.lower() in k.lower()}

    if matches:
        for term, meaning in matches.items():
            st.subheader(term)
            st.info(meaning)
    else:
        st.warning("No matching term found.")
else:
    st.caption("Start typing to search definitions…")
